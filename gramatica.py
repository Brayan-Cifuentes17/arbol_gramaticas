class Gramatica:
    ...
    def procesar_producciones(self, producciones):
        ...
        for produccion in producciones:
            izq, der = produccion.split("->")  # Separa la parte izquierda y derecha de la producción
            alternativas = der.split("|")  # cuadno existen diferentes producciones
            if izq in prod_dict:
            ...
            if simbolo in self.no_terminales:
                for produccion in self.producciones.get(simbolo, []):
                    if produccion == '':  # Si hay una producción que genera epsilon
                        nueva_palabra = actual.replace(simbolo, '', 1)  # Elimina el no terminal
                        if self.derivar(nueva_palabra, palabra, nivel + 1): 
                            return True
    ...
    def mostrar_arbol_general(self, simbolo=None, nivel=0, max_nivel=4):
        ...
        for i, s in enumerate(simbolo):
            if s in self.no_terminales:
                for produccion in self.producciones.get(s, []):
                    ...
                    arbol += self.mostrar_arbol_general(nueva_palabra, nivel + 1, max_nivel)  
